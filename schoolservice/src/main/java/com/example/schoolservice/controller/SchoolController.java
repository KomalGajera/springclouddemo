...
public class SchoolController {
	...
	public ResponseEntity<APIResponse> getStudentBySchool(@PathVariable long id) {

		School school=schoolService.getSchoolByid(id);
		APIResponse response = restTemplate.exchange("http://student-service/student/getstudentbyschoolid/{studentid}",
				HttpMethod.GET, null, new ParameterizedTypeReference<APIResponse>() {
		
		}, school.getId()).getBody();
		return ResponseEntity.ok(new APIResponse(HttpStatus.OK.value(), true,
				this.messages.getMessage("get school success", null, LocaleContextHolder.getLocale()), response.getData()));
	}
	
	public ResponseEntity<APIResponse> noStudentFoundFromSchool(@PathVariable long id) {

		String response="there are no student avaiable to this school...";
		return ResponseEntity.status(HttpStatus.OK).body(new APIResponse(HttpStatus.OK.value(), true,
				messages.getMessage(response, null, LocaleContextHolder.getLocale()), null));
	}
